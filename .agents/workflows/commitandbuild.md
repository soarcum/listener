...
Write-Host "�?版本号已更新�? $newVersionName ($newCode)"
...
## 步骤 2：监�?GitHub Actions 打包状�?
使用项目根目录下�?`.github_token` 来调�?GitHub API，轮询检查最近一次运行的工作流状态，直到它变�?`completed`�?
// turbo
```powershell
...
while ($true) {
    ...
    if ($run.status -eq "completed") {
        if ($run.conclusion -eq "success") {
            Write-Host "🎉 打包成功完成�?
            Write-Host "可以前往此处下载产物或查看详�? $($run.html_url)"
            break
        } else {
    ...
    Write-Host "当前状�? $($run.status)... 等待 10 秒后重新查询�?
